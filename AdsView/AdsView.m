...
# pragma mark-----内容视图
@implementation AdsContainerView
{
...
- (void)buildUI{
    ...
    closeButton                        = [AdsCloseButton buttonWithType:UIButtonTypeCustom];
...
- (void)showAnimated:(BOOL)animated{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(AdsViewDidAppear:)]) {
        [self.delegate AdsViewDidAppear:self];
    }
...
- (void)hideAnimated:(BOOL)animated{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(AdsViewDidDisAppear:)]) {
        [self.delegate AdsViewDidDisAppear:self];
    }
...
- (void)tapContainView:(UITapGestureRecognizer *)tapgesture{
    ...
    [self hideAnimated:YES];
    if (self.delegate && [self.delegate respondsToSelector:@selector(AdsViewTapMainContainView: currentSelectIndex:)]) {
        [self.delegate AdsViewTapMainContainView:self currentSelectIndex:_selectIndex];
    }
}
@end
